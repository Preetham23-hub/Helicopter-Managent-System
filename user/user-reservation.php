...
$id=$_GET['id'];  //may be jet id
	...
	$no=$rows_flight['no']; //admin no kottu flight add madovaga adru ticket price mathe class halirthane
	...
	$seats=$rows_seats['available']; //idna flight seats restrict ge use madbeku iga
...
if(isset($_POST['submit'])){
	...
	/* else if((empty($adult)  && empty($child)) || $adult<0 || $child<0 || $adult===0 || $child===0){
		$passenger_empty="<div class='error res-error'> Please Enter No.of Adult/Child Passengers or enter the valid passenger</div>"; */
else{
	...
	<form action="" method="POST" name="reservation-form" class="reservation-form">
		...
		<table class="user-reserv-table reservation-three">
			...
			<tr>
				...
				<td><p style="color: black; font-weight:bold;">seats available <?php echo "$sea"; ?></p> <label class="reservation-label">Adult</label> </td>
			</tr>
