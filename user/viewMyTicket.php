	...
	<div class="container">
		<div class="view-area">
					...
					<?php
						...
						$search_query="SELECT * FROM booking WHERE uname='$u_name'";

						$res=mysqli_query($con,$search_query);

						if($res){
							if(mysqli_num_rows($res)>0){
										echo "<div class='success count-div'>flights available<span class='flight-count'>".mysqli_num_rows($res)."</span></div>";
								...
								while($row = mysqli_fetch_array($res)) {
								    ...
								    echo "<td>
								    <a class='view-option' href='viewTicket.php?pnr=".$row['pnr']."'>View</a>
								    </td>";

								     echo "<td>
								    <a class='cancel-option' href='cancelTicket.php?pnr=".$row['pnr']."'>Cancel</a>
								    </td>";
